<properties 
    pageTitle="A folyamatos átvitelű forgatókönyv feladatátvevő végrehajtási |} Microsoft Azure" 
    description="Ez a témakör bemutatja a folyamatos átvitelű forgatókönyv feladatátvevő megvalósításáról." 
...
#<a name="implementing-failover-streaming-scenario"></a>A folyamatos átvitelű forgatókönyv feladatátvevő végrehajtása

Ez a forgatókönyv bemutatja, hogyan másolása (BLOB) tartalma tárgyi a másikba annak érdekében, hogy az igény szerinti streaming redundancia kezelni. Ebben az esetben célszerű azoknak a felhasználóknak, az a CDN között két adatközpontokkal esetén az egyik saját adatközpontokban egy üzemszünetek segítségével be szeretné állítani.
Ez a forgatókönyv használja a Microsoft Azure Media Services SDK, a Microsoft Azure Media Services REST API-t és Azure tároló SDK bemutatják, az alábbi műveleteket.

1. A Media Services fiók beállítása az "Adatközpont A".
1. Emeletes fájl feltöltése a forrás eszköz be.
1. Az eszköz kódolását több bit ráta MP4 fájlokba. 
1. Hozzon létre egy írásvédett Társítások megnevezés a forrás eszköz a tároló olvasási hozzáférést a forrás eszköz társított tárterület-fiókjában.
1. Az előző lépésben létrehozott csak olvasható Társítások megnevezés megnyithatja a forrás eszköz tároló nevét. Ez az információ szükséges az másolása BLOB-tároló-fiókok (magyarázata a témakörnek a későbbi szakaszában.) között
1. Hozzon létre egy origin megnevezés hozta létre a kódolási tevékenység eszköz. 

Ezt követően a feladatátvételi kezelése:

1. A Media Services fiók beállítása az "Adatok központ B".
1. Cél üres eszköz létrehozása a célhely Media Services-fiók.
1. Hozzon létre egy írási Társítások megnevezés a cél üres eszköz írási hozzáféréssel kell rendelkeznie a tárolóhoz a célhely a cél eszköz társított tárterület-fiókot.
1. Azure tároló SDK segítségével BLOB (digitáliseszköz-fájlok) másolása a forrás tárterület-fiók "Adatközpont A" és "Adatok központ B" cél tárterület-fiók között (a tárhely fiókok társítva az érdeklődésre számot tartó eszközök.)
1. Rendelje hozzá a cél blob-tárolóban a cél eszköz kimásolt BLOB (digitáliseszköz-fájlok). 
1. Hozzon létre egy origin megnevezés a eszköz "Adatok központ B", és adja meg a megnevezés ID azonosítója, a "Adatközpont A" eszköz jött létre. 
1. Ezzel kap adatfolyam URL-címét az URL-címek relatív elérési esetén azonos (csak a következő URL-címek eltérőek). 
 
Ezt követően bármilyen kimaradások kezelendő is létrehozhat a következő origin Locator fölött CDN. 

A következő érvényesek:

- Az aktuális verziójának Media Services SDK nem támogatja a egy megnevezés létrehozása egy megadott megnevezés azonosítóval. Ebben a feladatban eléréséhez fogjuk használni Media Services REST API-t.
- Az aktuális verziójának Media Services SDK nem támogatja a tárgyi eszköz szeretne társítani eszköz fájlok programozás útján létrehozása IAssetFile információkat. Ebben a feladatban eléréséhez fogjuk használni a CreateFileInfos Media Services REST API-t. 
- Titkosított tároló eszközök (AssetCreationOptions.StorageEncrypted) nem támogatott a replikáció (óta lesz, hogy a titkosítási kulcs más a mindkét Media Services fiókok). 
- Szeretne kihasználhatja dinamikus csomagolása, ha legalább egy önálló a folyamatos átvitelű fenntartott egységek először be kell szereznie. További tudnivalókért lásd: [Dinamikusan termékcsomagolás eszközöket](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Fontolja meg inkább a Media Services [Replikációs eszköz](http://replicator.codeplex.com/) feladatátvevő adatfolyam példa manuálisan végrehajtási alternatívájaként. Az eszközben két Media Services fiókok közötti eszközök replikáció teszi lehetővé.

##<a name="prerequisites"></a>Előfeltételek
 
- Két a Media Services fiókok új vagy meglévő Azure-előfizetésben. Megtudhatja, [hogy miként Media Services-fiók létrehozása](media-services-portal-create-account.md).
- Operációs rendszer: A Windows 7, Windows 2008 R2 vagy Windows 8-ban.
- .NET-keretrendszer 4.5 vagy a .NET-keretrendszer 4.
- Visual Studio 2010 SP1 vagy újabb verzióját (Professional, prémium, Ultimate vagy Express).
 
##<a name="set-up-your-project"></a>A projekt beállítása

Ez a szakasz létrehozása, és C# New-projekt beállítása.

1. Hozzon létre egy új megoldás, amely tartalmazza a C# New project a Visual Studio segítségével. Írja be a HandleRedundancyForOnDemandStreaming a jelölőnégyzetét, és kattintson az OK gombra.
1. A SupportFiles mappa létrehozása a HandleRedundancyForOnDemandStreaming.csproj projektfájl azonos szintre. A SupportFiles mappán hozza létre a OutputFiles és MP4Files mappákat. Másolja az .mp4 kiterjesztésű MP4Files mappába (a példában a BigBuckBunny.mp4 fájlt használ). 
1. Kövesse **Nuget** Media Services kapcsolódó DLL-ek mutató hivatkozásokat. Visual Studio fő menüben válassza az eszközök-tár csomag kezelő > csomag Manager Console ->. Konzol ablakban írja a telepítés-csomag windowsazure.mediaservices, és nyomja le a írja.
1. Adja hozzá a többi hivatkozásának szükséges ehhez a projekthez: System.Configuration System.Runtime.Serialization és System.Web.
1. A csere, amelyeket felvettek a Programs.cs fájl alapértelmezés szerint a következő lehetőségekből együtt kimutatások használatával:
...
1. A appSettings szakasz hozzáadása a .config fájlt, és frissítse az értékek alapján a Media Services és tárolási használatával és a nevek értékei. 
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Adja hozzá az igény szerinti streaming redundancia kezelő kódot.



1. Az alábbi osztály szintű mezők hozzáadása a Program osztály.
...
1. Az alapértelmezett fő módszer definíciója cserélje ki a következő lehetőségek:
...
1. A fő úgynevezett módszer definíciókat alatti határozzák meg.
...
A két adatközpontokban, és így esetén minden kimaradások feladatátvevő között kérelmek forgalom kezelője most már használhatja.
