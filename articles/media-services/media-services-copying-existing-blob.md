<properties 
    pageTitle="Egy meglévő Blob másolása beillesztheti egy szolgáltatások eszköz |} Microsoft Azure" 
    description="Ez a témakör bemutatja, hogyan egy meglévő blob másolja a Media Services eszköz." 
...
#<a name="copying-an-existing-blob-into-a-media-services-asset"></a>Egy meglévő Blob másolja a Media Services eszköz

Ez a témakör bemutatja, hogyan BLOB másol egy új Microsoft Azure Media Services eszköz tárterület-fiókból.

A BLOB egy Media Services-fiókot, illetve a tárhely fiókhoz, nem Media Services fiókjához társított társított tárterület-fiókot is szerepel. Ez a témakör bemutatja, hogyan lehet BLOB másolja a Media Services eszköz tárterület-fiókból. Figyelje meg, hogy is másolhat át adatközpontokban. Lehetnek, így felmerült költségeket. Árak kapcsolatos további tudnivalókért olvassa el az [Adatok átvitele](https://azure.microsoft.com/pricing/#header-11)című témakört.
...
Szerezze be és lebonyolítása minta [Itt](https://azure.microsoft.com/documentation/samples/media-services-dotnet-copy-blob-into-asset/).

##<a name="prerequisites"></a>Előfeltételek

- Két a Media Services fiókok új vagy meglévő Azure-előfizetésben. Olvassa el a [Media Services-fiók létrehozása](media-services-portal-create-account.md).
- Operációs rendszer: A Windows 10, Windows 7, Windows 2008 R2 vagy Windows 8-ban.
- .NET-keretrendszer 4.5.
- Visual Studio 2010 SP1 (Professional, prémium, Ultimate vagy Express) vagy újabb verziójában.

##<a name="set-up-your-project"></a>A projekt beállítása

Ez a szakasz létrehozása, és C# New-projekt beállítása.

1. Hozzon létre egy új megoldás, amely tartalmazza a C# New project a Visual Studio segítségével. 
2. Írja be a CopyExistingBlobsIntoAsset a jelölőnégyzetét, és kattintson az OK gombra.
1. A kapcsolódó DLL-ek használatát Nuget Media Services mutató hivatkozásokat adhat. Visual Studio fő menüben válassza az eszközök-tár csomag kezelő > csomag Manager Console ->. Konzol ablakban írja a telepítés-csomag windowsazure.mediaservices, és nyomja le a írja.
1. Adja hozzá a többi hivatkozásának szükséges ehhez a projekthez: System.Configuration.
...
##<a name="copy-blobs-from-a-storage-account-into-a-media-services-asset"></a>BLOB másolja be a Media Services eszköz tároló fiókból

Az alábbi példa az alábbi műveleteket végzi el:

1. A CloudMediaContext példányt hoz. 
1. Hoz létre CloudStorageAccount példányok: _sourceStorageAccount és _destinationStorageAccount.
1. Zökkenőmentes Streaming fájlok helyi könyvtárból feltölti _sourceStorageAccount található blob tárolóba. 
1. Létrehoz egy új eszközre. A blob-tárolóhoz, amely a digitáliseszköz-készült található _destinationStorageAccount. 
1. Azure tároló SDK használja a megadott BLOB másolja a tároló az eszköz társított.

    >[AZURE.NOTE]A másolás nem kivételhibát, ha a megnevezés lejárt.

1. Óta, ebben a példában zökkenőmentes adatfolyam fájlokat, másolja azt a példa bemutatja, hogyan állíthatja a .ism fájlt, az elsődleges fájl lesz. Ha például azt az .mp4 kiterjesztésű másolja, a mp4 fájlt szeretne beállítani az elsődleges fájl lehet.
1. Létrehoz az eszköz társított OnDemandOrigin megnevezés zökkenőmentes Streaming URL-CÍMÉT. 
