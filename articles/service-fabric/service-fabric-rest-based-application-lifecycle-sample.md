<properties
   pageTitle="TÖBBI-alapú alkalmazás életciklus minta |} Microsoft Azure"
   description="Microsoft Azure Service háló minta, amely mutatja az alkalmazás életciklus a szolgáltatás háló többi felhasználói felület használatával."
...
# <a name="rest-based-application-lifecycle-sample"></a>TÖBBI-alapú alkalmazás életciklus-minta

Az alábbi példa bemutatja a szolgáltatás háló alkalmazás életciklusáról keresztül REST API-hívások. A szolgáltatás háló alkalmazás életciklusáról kapcsolatos további tudnivalókért olvassa el a [szolgáltatás háló alkalmazás életciklus](service-fabric-application-lifecycle.md)című témakört.

Ez a példa hajtja végre a következőket:

* Rendelkezések WordCount alkalmazáscsomag a képen a **WordCount 1.0.0** mintája tárolni.
* Alkalmazás-típusokat, WordCount 1.0.0 tartalmazó listáját jeleníti meg.
* Hoz létre WordCount alkalmazása **háló: / WordCount**.
* Az alkalmazáslistából, amelyek tartalmazzák még a háló jeleníti meg: / WordCount 1.0.0 verziója.
* A kép áruházban **WordCountUpgrade** alkalmazáscsomag a WordCount mintája rendelkezések a 1.1.0 verziója.
* Listája alkalmazás típusú WordCount 1.0.0 és **WordCount 1.1.0**is beleértve.
* Frissíti a WordCount alkalmazás 1.1.0 verzióra.
* Alkalmazások, amelyek WordCount verzió 1.1.0 tartalmazza, de már nem tartalmazza a WordCount verzió 1.0.0 listáját jeleníti meg.
* Törli az WordCount alkalmazást.
* Az alkalmazáslistából, amely már nem tartalmazza a háló jeleníti meg: / WordCount.
* A WordCount minta unprovisions a 1.1.0 verziója.
* Alkalmazás típusú, amely magában foglalja a WordCount 1.0.0, de már nem tartalmazza a WordCount 1.1.0 listáját jeleníti meg.
* A WordCount minta unprovisions a 1.0.0 verziója.
* Alkalmazás típusú, amely már nem tartalmazza a WordCount listáját jeleníti meg.


## <a name="prerequisites"></a>Előfeltételek

Ez a példa a [WordCount minta](http://aka.ms/servicefabricsamples) (az **Első lépések** minták található) használja. A WordCount minta először kell beépített, és ezt követően két alkalmazáscsomagok kell másolni a kép áruházból.

|Mappa|Leírás|
|------|-----------|
|WordCount|A minta WordCount alkalmazást. A **ApplicationManifest.xml** fájl tartalmaz **ApplicationTypeVersion = "1.0.0"**.|
|WordCountUpgrade|A minta WordCount alkalmazást. A ApplicationManifest.xml fájl kell változtatni **ApplicationTypeVersion = "1.1.0"** fordulhat elő, hogy az alkalmazás frissítés engedélyezése.|

Az alkalmazáscsomagok létrehozásához, és másolja a képet áruházból, kövesse az alábbi lépéseket:

1. Másolja a **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** **C:\Temp\WordCount**. Ez a WordCount alkalmazáscsomag létrehozása
2. Másolja a C:\Temp\WordCount **C:\Temp\WordCountUpgrade**. Ez a **WordCountUpgrade** alkalmazáscsomag létrehozása
3. Nyissa meg a **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** szövegszerkesztőben.
4. **ApplicationManifest** eleme módosítsa a **ApplicationTypeVersion** attribútum **"1.1.0"**.  Ezzel frissíti az alkalmazás verziószámát.
5. Mentse a módosított ApplicationManifest.xml fájlt.
6. Futtassa az alábbi PowerShell parancsprogramot másolja a vágólapra a kép áruház az alkalmazások rendszergazdaként:
...
Amikor befejezte a PowerShell-parancsprogramot, ez az alkalmazás készen áll a futtatásához.

## <a name="example"></a>Példa

Az alábbi példa bemutatja a szolgáltatás háló alkalmazás életciklusáról.
