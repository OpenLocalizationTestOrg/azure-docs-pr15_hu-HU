...
Ez a témakör bemutatja a PowerShell-parancsmagok, amely számos Azure SQL-adatbázissal kapcsolatos feladat végzik. Teljes listáját olvassa el a [Azure SQL adatbázis parancsmagok] (https://msdn.microsoft.com/library/mt574084(v=azure.300\).aspx).


## <a name="create-a-resource-group"></a>Erőforráscsoport létrehozása

Erőforráscsoport létrehozása az SQL-adatbázis és a kapcsolódó Azure erőforrások az [új-AzureRmResourceGroup] (https://msdn.microsoft.com/library/azure/mt759837(v=azure.300\).aspx) parancsmag.
...
Hozzon létre egy SQL-adatbázis kiszolgálója és az [új-AzureRmSqlServer] (https://msdn.microsoft.com/library/azure/mt603715(v=azure.300\).aspx) parancsmag. *Kiszolgáló1* cserélje le a a kiszolgáló nevét. Kiszolgáló nevének egyedinek kell lennie a összes Azure SQL-adatbázis-kiszolgálón keresztül. Ha a kiszolgálónév már származik, hibaüzenetet kap. Ez a parancs eltarthat néhány percig, amíg befejezéséhez. Az erőforráscsoport már léteznie kell az előfizetést.
...
Az [új-AzureRmSqlServerFirewallRule] (https://msdn.microsoft.com/library/azure/mt603860(v=azure.300\).aspx) parancsmag, a kiszolgáló elérésére tűzfal szabály létrehozása. A következő parancsot, a kezdő és záró IP-címek helyettesítve érvényes értékeket az ügyfél számára. Az erőforráscsoport és a kiszolgáló már léteznie kell az előfizetést.
...
Adatbázis létrehozása az [új-AzureRmSqlDatabase] (https://msdn.microsoft.com/library/azure/mt619339(v=azure.300\).aspx) parancsmag. Az erőforráscsoport és a kiszolgáló már léteznie kell az előfizetést. 
...
SQL-adatbázis másolása ugyanazon a kiszolgálón az [új-AzureRmSqlDatabaseCopy] (https://msdn.microsoft.com/library/azure/mt603644(v=azure.300\).aspx) parancsmag. Állítsa a `-CopyServerName` és `-CopyResourceGroupName` csoportként a forrás adatbázis-kiszolgáló és az erőforrás azonos értékre.
...
A [eltávolítása-AzureRmSqlDatabase] (https://msdn.microsoft.com/library/azure/mt619368(v=azure.300\).aspx) parancsmag az SQL-adatbázis törlése. Az erőforráscsoport, a kiszolgáló és az adatbázis már léteznie kell az előfizetést.
...
Törölje a kiszolgáló és az [eltávolítása-AzureRmSqlServer] (https://msdn.microsoft.com/library/azure/mt603488(v=azure.300\).aspx) parancsmag.
...
- [Azure parancsmagjai – referencia] (https://msdn.microsoft.com/library/azure/dn708514(v=azure.300\).aspx)
